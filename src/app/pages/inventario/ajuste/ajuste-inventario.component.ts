...
export default class AjusteInventarioPage implements OnInit {
  ...
  form = new FormGroup({
    ...
    permitirNegativo: new FormControl<boolean>(false, { nonNullable: true }),
  });
  ...
  private loadIngredientes(): void {
    ...
    forkJoin({
      ...
      next: ({ ingredientes, inventario }) => {
        ...
        // Crear Map de ingredientes por ID para acceso rápido
        const ingredientesMap = this.crearMapaIngredientes(ingredientesActivos);

        // Combinar datos de inventario con datos de ingredientes
        this.ingredientes = this.combinarInventarioConIngredientes(
          inventario.contenido ?? inventario.content ?? [],
          ingredientesMap
        );
  ...
  private crearMapaIngredientes(ingredientes: any[]): Map<number, { nombre: string; codigo: string; unidad: string }> {
    return new Map(
  ...
  private combinarInventarioConIngredientes(
    inventarioArr: any[],
    ingredientesMap: Map<number, { nombre: string; codigo: string; unidad: string }>
  ): Array<{ id: number; nombre: string; codigo: string; stockActual: number; unidad: string }> {
    return inventarioArr
      .map((inv: any) => {
  ...
  onSubmit(): void {
    ...
    const ajuste: AjusteInventario = {
      ...
      permitirNegativo: value.permitirNegativo ?? false,
    };
